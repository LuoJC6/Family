...
public class ManageFlag extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.manage_flag);// 设置布局文件
		txtFlag = (EditText) findViewById(R.id.txtFlagManage);// 获取便签文本框
		btnEdit = (Button) findViewById(R.id.btnFlagEdit);// 获取修改按钮
		btnDel = (Button) findViewById(R.id.btnFlagDelete);// 获取删除按钮

		Intent intent = getIntent();// 创建Intent对象
		Bundle bundle = intent.getExtras();// 获取便签id
		strid = bundle.getString(Showinfo.FLAG);// 将便签id转换为字符串
		final FlagDAO flagDAO = new FlagDAO(ManageFlag.this);// 创建FlagDAO对象
